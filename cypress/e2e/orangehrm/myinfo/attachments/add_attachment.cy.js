...
describe('My Info - Attachments - Add (@attachments)', () => {
  it('Sube un adjunto con comentario', () => {
    cy.loginOrange();
    ...
    const comment = `QA add ${Date.now()}`;
    my.addAttachment('cypress/fixtures/sample.png', comment);

    cy.wait('@listAttach', { timeout: 15000 })
      .its('response.statusCode')
      .should('eq', 200);

    cy.contains('.oxd-table-card, .oxd-table-row', comment, { timeout: 15000 })
      .should('be.visible');
  });
});